var tinymce={majorVersion:"3",minorVersion:"2.4.1",releaseDate:"2009-05-25",_init:function(){var L=this,I=document,J=window,H=navigator,B=H.userAgent,G,A,F,E,D,K;
...
},split:function(L,K,O){var P=this,F=P.createRng(),M,J,N;
function G(R,Q){R=R[Q];
if(R&&R[Q]&&R[Q].nodeType==1&&I(R[Q])){P.remove(R[Q])
}}function I(Q){Q=P.getOuterHTML(Q);
Q=Q.replace(/<(img|hr|table)/gi,"-");
Q=Q.replace(/<[^>]+>/g,"");
return Q.replace(/[ \t\r\n]+|&nbsp;|&#160;/g,"")==""
}function H(R){var Q=0;
while(R.previousSibling){Q++;
R=R.previousSibling
}return Q
}if(L&&K){F.setStart(L.parentNode,H(L));
...
N=L.parentNode;
G(M,"lastChild");
if(!I(M)){N.insertBefore(M,L)
}if(O){N.replaceChild(O,K)
}else{N.insertBefore(K,L)
}G(J,"firstChild");
if(!I(J)){N.insertBefore(J,L)
}P.remove(L);
return O||K
