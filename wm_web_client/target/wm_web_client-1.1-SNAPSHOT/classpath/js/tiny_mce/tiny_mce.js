var tinymce={majorVersion:"3",minorVersion:"2.3",releaseDate:"2009-04-23",_init:function(){var A=this,D=document,C=window,E=navigator,K=E.userAgent,F,L,G,H,I,B;
...
this.settings=H=E.extend({keep_values:false,hex_colors:1,process_html:1},H);
...
break;
default:if(F.indexOf("on")===0&&J){J=(""+J).replace(/^function\s+anonymous\(\)\s+\{\s+(.*)\s+\}$/,"$1")
...
return G
},loadCSS:function(H){var G=this,F=G.doc;
if(!H){H=""
}C(H.split(","),function(I){if(G.files[I]){return 
}G.files[I]=true;
G.add(G.select("head")[0],"link",{rel:"stylesheet",href:E._addVer(I)})
})
...
return J
}F=F.replace(/<script([^>]+|)>([\s\S]*?)<\/script>/g,function(J,K,L){L=I(L);
if(!K){K=' type="text/javascript"'
}if(L){L="<!--\n"+L+"\n// -->"
}return"<mce:script"+K+">"+L+"</mce:script>"
});
F=F.replace(/<style([^>]+|)>([\s\S]*?)<\/style>/g,function(J,K,L){L=I(L);
return"<mce:style"+K+"><!--\n"+L+"\n--></mce:style><style"+K+' mce_bogus="1">'+L+"</style>"
})
...
G.win=G.doc=G.root=null;
if(!F){E.removeUnload(G.destroy)
}},createRng:function(){var F=this.doc;
return F.createRange?F.createRange():new E.dom.Range(this)
},split:function(J,K,G){var F=this,O=F.createRng(),I,L,H;
function N(P,Q){P=P[Q];
if(P&&P[Q]&&P[Q].nodeType==1&&M(P[Q])){F.remove(P[Q])
}}function M(P){P=F.getOuterHTML(P);
P=P.replace(/<(img|hr|table)/gi,"-");
P=P.replace(/<[^>]+>/g,"");
return P.replace(/[ \t\r\n]+|&nbsp;|&#160;/g,"")==""
}if(J&&K){O.setStartBefore(J);
O.setEndBefore(K);
I=O.extractContents();
O=F.createRng();
O.setStartAfter(K);
...
E.create("static tinymce.dom.Event",{inits:[],events:[],add:function(G,N,H,J){var M,L=this,K=L.events,I;
...
if(!G){return 
}M=function(O){O=O||window.event;
if(O&&!O.target&&A){O.target=O.srcElement
}if(!J){return H(O)
}return H.call(J,O)
};
...
}}}catch(J){}}},_pageInit:function(){var G=B;
if(G.domLoaded){return 
}G._remove(window,"DOMContentLoaded",G._pageInit);
G.domLoaded=true;
C(G.inits,function(H){H()
});
G.inits=[]
},_wait:function(){if(window.tinyMCE_GZ&&tinyMCE_GZ.loaded){B.domLoaded=1;
return 
...
if(document.documentElement.doScroll&&window==window.top){(function(){if(B.domLoaded){return 
}try{document.documentElement.doScroll("left")
}catch(G){setTimeout(arguments.callee,0);
return 
}B._pageInit()
})()
}}else{if(document.addEventListener){B._add(window,"DOMContentLoaded",B._pageInit,B)
}}B._add(window,"load",B._pageInit,B)
}});
B=E.dom.Event;
B._wait();
E.addUnload(B._unload)
})(tinymce);
...
},_serializeNode:function(O,V){var I=this,H=I.settings,L=I.writer,T,X,P,M,N,K,S,Y,J,W,R,Q,U;
if(!H.node_filter||H.node_filter(O)){switch(O.nodeType){case 1:if(O.hasAttribute?O.hasAttribute("mce_bogus"):O.getAttribute("mce_bogus")){return 
}U=false;
T=O.hasChildNodes();
W=O.getAttribute("mce_name")||O.nodeName.toLowerCase();
if(F){if(O.scopeName!=="HTML"&&O.scopeName!=="html"){W=O.scopeName+":"+W
}}if(W.indexOf("mce:")===0){W=W.substring(4)
}if(!I.validElementsRE.test(W)||(I.invalidElementsRE&&I.invalidElementsRE.test(W))||V){U=true;
break
}if(F){if(H.fix_content_duplication){if(O.mce_serialized==I.key){return 
}O.mce_serialized=I.key
}if(W.charAt(0)=="/"){W=W.substring(1)
}}else{if(B){if(O.nodeName==="BR"&&O.getAttribute("type")=="_moz"){return 
}}}if(I.childRules){if(I.parentElementsRE.test(I.elementName)){if(!I.childRules[I.elementName].test(W)){U=true;
break
}}I.elementName=W
}R=I.findRule(W);
W=R.name||W;
if((!T&&R.noEmpty)||(F&&!W)){U=true;
break
}if(R.requiredAttribs){K=R.requiredAttribs;
for(M=K.length-1;
M>=0;
M--){if(this.dom.getAttrib(O,K[M])!==""){break
}}if(M==-1){U=true;
break
}}L.writeStartElement(W);
if(R.attribs){for(M=0,S=R.attribs,N=S.length;
M<N;
M++){K=S[M];
J=I._getAttrib(O,K);
if(J!==null){L.writeAttribute(K.name,J)
}}}if(R.validAttribsRE){S=I.dom.getAttribs(O);
for(M=S.length-1;
M>-1;
M--){Y=S[M];
if(Y.specified){K=Y.nodeName.toLowerCase();
if(H.invalid_attrs.test(K)||!R.validAttribsRE.test(K)){continue
}Q=I.findAttribRule(R,K);
J=I._getAttrib(O,Q,K);
if(J!==null){L.writeAttribute(K,J)
}}}}if(R.padd){if(T&&(P=O.firstChild)&&P.nodeType===1&&O.childNodes.length===1){if(P.hasAttribute?P.hasAttribute("mce_bogus"):P.getAttribute("mce_bogus")){L.writeText("\u00a0")
}}else{if(!T){L.writeText("\u00a0")
}}}break;
case 3:if(I.childRules&&I.parentElementsRE.test(I.elementName)){if(!I.childRules[I.elementName].test(O.nodeName)){return 
}}return L.writeText(O.nodeValue);
case 4:return L.writeCDATA(O.nodeValue);
case 8:return L.writeComment(O.nodeValue)
}}else{if(O.nodeType==1){T=O.hasChildNodes()
}}if(T){P=O.firstChild;
while(P){I._serializeNode(P);
I.elementName=W;
P=P.nextSibling
}}if(!U){if(T||!H.closed.test(W)){L.writeFullEndElement()
}else{L.writeEndElement()
...
}}G(Q,function(Y,Z){switch(Z){case"contextmenu":if(C.isOpera){F.add(R.getBody(),"mousedown",function(a){if(a.ctrlKey){a.fakeType="contextmenu";
T(a)
}})
}else{F.add(R.getBody(),Z,T)
}break;
case"paste":F.add(R.getBody(),Z,function(a){T(a)
});
break;
case"submit":case"reset":F.add(R.getElement().form||B.getParent(R.id,"form"),Z,T);
break;
default:F.add(X.content_editable?R.getBody():R.getDoc(),Z,T)
}});
F.add(X.content_editable?R.getBody():(P?R.getDoc():R.getWin()),"focus",function(Y){R.focus(true)
});
if(C.isGecko){F.add(R.getDoc(),"DOMNodeInserted",function(Y){var Z;
Y=Y.target;
...
}if(C.isIE){F.add(R.getDoc(),"controlselect",function(Y){var Z=R.resizeInfo,a;
Y=Y.target;
if(Y.nodeName!=="IMG"){return 
}if(Z){F.remove(Z.node,Z.ev,Z.cb)
}if(!R.dom.hasClass(Y,"mceItemNoResize")){ev="resizeend";
a=F.add(Y,ev,function(b){var c;
b=b.target;
...
a=F.add(Y,"resizestart",F.cancel,F)
...
R.undoManager.add()
}if(C.isIE){F.add(R.getWin(),"blur",function(Z){var Y;
if(R.selection){Y=R.selection.getNode();
if(!R.removed&&Y.ownerDocument&&Y.ownerDocument!=R.getDoc()){U()
}}})
}else{F.add(R.getDoc(),"blur",function(){if(R.selection&&!R.removed){U()
}})
}R.onMouseDown.add(U);
...
return N
}if(G){L.onPreProcess.add(function(M,N){E(M.dom.select("p,h1,h2,h3,h4,h5,h6,div",N.node),function(O){if(D(O)){O.innerHTML=""
}})
});
if(J.element!="P"){L.onKeyPress.add(function(M,N){K.lastElm=M.selection.getNode().nodeName
});
...
if(a===AG&&/^(TD|TH)$/.test(a.nodeName)){d.remove(a.firstChild);
AE.dom.add(a,S.element,null,"<br />");
...
if(E.isIE){I.onHideMenu.add(function(){if(M){J.selection.moveToBookmark(M);
M=0
}})
}return H.add(I)
